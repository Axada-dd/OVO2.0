...
public class 开满转火: ISlotSequence
{
    ...
    public int StartCheck()
    {
        ...
        if (BlackMageHelper.三目标aoe() || BlackMageHelper.双目标aoe()) return -7;
        ...
        if (!BlackMageHelper.冰状态) return -3;
        if (BlackMageHelper.冰层数 != 3) return -4;
        if (Helper.有buff(Buffs.三连Buff) || Helper.有buff(Buffs.即刻Buff)) return -7;
        if (BlackMageHelper.冰针 == 3 || Skill.冰澈.RecentlyUsed() || Skill.冰冻.RecentlyUsed()) return -6;
        return 1;
    }
    ...
    private static void Step0(Slot slot)
    {
        ...
        if (BlackMageHelper.通晓层数 == 3 && BlackMageHelper.通晓剩余时间 < 13)
            slot.Add(new Spell(aoe ? Skill.秽浊 : Skill.异言, SpellTargetType.Target).DontUseGcd());
        if (BlackMageHelper.提前补dot() && BlackMageQT.GetQt(QTkey.Dot) && Helper.有buff(Buffs.雷云))
        {
