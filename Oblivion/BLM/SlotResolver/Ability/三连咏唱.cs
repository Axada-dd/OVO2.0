...
public class 三连咏唱 : ISlotResolver
{
    ...
    public int Check()
    {
        ...
        if (BlackMageHelper.火状态)
        {
            ...
            {
                if (Core.Me.CurrentMp < 800 && BlackMageHelper.耀星层数 == 6) return -22;
                return 23;
            }

            if (Skill.墨泉.技能CD() < 500 || Skill.墨泉.AbilityCoolDownInNextXgcDsWindow(5)) return -8;
            if (Core.Me.CurrentMp <= 4400 && BlackMageHelper.耀星层数 >= 5 && Core.Me.Level == 100&&Skill.即刻.技能CD()>0&&!Skill.即刻.AbilityCoolDownInNextXgcDsWindow(3)) return 1;
        ...
        if (BlackMageHelper.冰状态 && BlackMageHelper.冰层数 < 3)
        {
            ...
            if (BlackMageHelper.悖论指示 ) return -3;
            return 2;
        }
        return -99;
    }
