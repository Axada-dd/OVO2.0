...
public class 火单100 :ISlotResolver
{
    ...
    {
        if (BlackMageHelper.冰状态) return 0;
        if (!BlackMageHelper.火状态) return 0;
        if (!BlackMageHelper.火状态 && Helper.蓝量 > 8000) return Skill.火三;
        if (BlackMageHelper.火层数 < 3)
        {
            if (BlackMageHelper.有火苗)
                return Skill.火三;
            if (BlackMageHelper.悖论指示)
                return Skill.悖论;
        }
        ...
        if (BattleData.Instance.压缩冰悖论 && Core.Me.CurrentMp <= 2800 && BlackMageHelper.耀星层数 < 5) return Skill.绝望;
        if (Helper.蓝量 < 2400 && BlackMageHelper.耀星层数 < 5 ) return Skill.绝望;
        return Skill.火四;
    }
    public int Check()
    {
