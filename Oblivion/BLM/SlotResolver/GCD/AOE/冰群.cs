    ...
    private uint GetSkillId()
    {
        if (BlackMageHelper.冰状态)
        {
            ...
            if (!Skill.星灵移位.GetSpell().IsReadyWithCanCast())
            {
                ...
                if (BlackMageHelper.通晓层数 > 0) return Skill.秽浊;
        ...
        if (BlackMageHelper.火状态)
        {
            ...
            if (Core.Me.CurrentMp < 800) return BlackMageHelper.三目标aoe() ? Skill.冰冻.GetActionChange() : Skill.冰三;
        }
        if (!BlackMageHelper.冰状态 && !BlackMageHelper.火状态) return Skill.冰冻;
        return 0;
    }
    public int Check()
    {
